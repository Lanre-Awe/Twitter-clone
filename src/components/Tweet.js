...
const Tweet = (props) => {
  ...
  const tweetComment = profile[0].tweet[0].comment;
  let counter = tweetLike;
  const REPLY = props.reply;

  const dispatch = useDispatch();
  const profileInfo = useSelector((state) => state.profile.profile);
  const retweetedTweet = props.tweeted.filter((item) => item.id === props.id)[0]
    .retweeted;
  ...
  const [commentCounter, setCommentCounter] = useState(tweetComment);
  const [retweeted, setRetweeted] = useState(retweetedTweet);
    ...
    <Fragment>
      ...
      <div className={classes.container}>
        <div className={classes.infoContainer}>
          {retweeted && <span className={classes.info}>You Retweeted</span>}
        </div>
        ...
        <div className={classes.majorIconContainer}>
          <button className={classes.mainContainer} onClick={commentHandler}>
            ...
            <span className={classes.commentCounter}>{commentCounter}</span>
          </button>
          <button className={classes.mainContainer} onClick={retweet}>
            ...
            <span className={classes.retweetCounter}>23</span>
          </button>
          <button
        ...
        {REPLY.map((replies) => (
          <Reply
            ...
            reply={replies.reply}
            id={replies.id}
            allReplies={props.reply}
            profileId={props.profileId}
            pfp={props.replyProfile[0].profilepicture}
            name={props.replyProfile[0].name}
            username={props.replyProfile[0].username}
            onIdentify={props.onIdentify}
            comment={props.comment}
            details={props.details}
            profile={props.profile}
            onRetweet={props.onRetweet}
            onRemove={deleteReplyHandler}
          />
        ))}
      </div>
    </Fragment>
